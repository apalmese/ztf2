...
For its near-term peculiar-velocity program, LBL advocates a survey using SN~Ia discoveries from ZTF2.
The reasons why:
\begin{itemize}
\item A peculiar-velocity survey is already being touted as a primary science motivator,  As such,
the observing strategy should accommodate our needs.
\item It is ready to start soon.  The infrastructure is in place.
\item It is cheap, with a cost ranging from zero to access public data, to an amount smaller
than building a new facility.
\item SEDMachine available for SN classification.
\item It is in the Northern Hemisphere, which complements and is not superseded by LSST.
\item It goes to sufficient depth to have a scientifically interesting result near term.
\end{itemize}
