...
where $\Omega_M(a)$ \replaced[id=2]{also depends on the gravity model}{depends on expansion rate determined by gravity}.
...
\caption{\replaced[id=2]{Dimensionless}{Volume-weighted} peculiar velocity power spectrum $k^3P_{vv}(z=0)$ for $\mu \equiv \cos{(\hat{k} \cdot \hat{r})}=1, 0.5$ (magenta, cyan) 
...
is \replaced{sufficient}{necessary} to ensure that statistical uncertainties are negligible.
...
\end{equation}
and the
\added[id=1]{peculiar-velocity  uncertainty  ($\sigma$) is related to magnitude uncertainty via $\sigma_M^2 =  \left(\frac{5}{\ln{10}}\right)^2 \left(1-\frac{1}{Ha\chi}\right)^2 \sigma^2$.}
